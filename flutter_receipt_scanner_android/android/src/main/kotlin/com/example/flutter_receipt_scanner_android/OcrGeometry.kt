 ...
 * See the native port map §6 (OCR angle rotation + line geometry).
 *
 * Pure Kotlin — no `android.graphics` types — so it stays JVM-unit-testable
 * without Robolectric, matching the RN `OcrGeometry` it ports. The angle helpers
 * are deliberately platform-agnostic: the Swift `OcrGeometry` mirrors the same
 * formulas on iOS, and the shared sign convention is what pins them together.
 */
object OcrGeometry {
     ...
     * [ImageProcessor.rotate] applies to the pixels. iOS rotates the *other* way
     * for the same degree value — see the native port map §6.
     */
    fun rotateClockwise(
        box: Box,
        frameWidth: Int,
